document.addEventListener("DOMContentLoaded", (e) => {
  ...
  const dialogItemContent =
    document.querySelectorAll(".dialogItemContent");
  ...
  for (let i = 0; i < diaHD.length; i++) {
    const item = diaHD[i];

    item.addEventListener('click', (e) => {
      dialogItemContent.forEach(item => item.classList.add('hide'))
      dialogItemContent[i].classList.remove('hide');
      fullScroll();
    })
  ...
  const allListItemsLeft = document.querySelectorAll(
    ".sidebarListItem svg path"
  );
